...
def main():
    """
    Main function to interactively find the most similar content from a set of embeddings based on user input questions.

    The function executes the following steps:
    1. Initializes the environment and settings using the `init` function.
    2. Reads a CSV file containing precomputed embeddings into a pandas DataFrame from a path specified in the global `config`.
    3. Continuously prompts the user to input a question until the user types 'exit'.
    4. For each question, it identifies the top 3 most similar pieces of content from the embeddings using cosine similarity.
    5. Displays the best matches to the user.

    Uses:
        - The embeddings should be precomputed and stored in a CSV file specified by the `vector_file` key in the config.
        - User questions are compared against these embeddings to find the most relevant content.

    Side Effects:
        - Outputs the comparison results to the console.
        - Modifies global state by using a configuration setup through `init`.
    """
    init()
    ...
    while question.strip().lower() != 'exit':
        ...
        print_best_matches(best_matches=best_matches)
        question = get_user_question()
...
def get_user_question() -> str:
    ...
    print("And we'll find the sections of the website")
    print("that are most likely to have relevant information.\n")
